...
if (isset($_POST['add_room'])) {
    ...
    if (insert($q1, $values, 'siiiiis')) {
        $flag = 1;
        $room_id = mysqli_insert_id($con); // Set room_id only if insert is successful
    } else {
        die('Room insert failed');
    }

    // Insert into room_facilities table
    ...
    if ($stmt = mysqli_prepare($con, $q2)) {
        ...
        $flag = 0;
        die('Query cannot be prepared for facilities insert');
    }

    // Insert into room_features table
    $q3 = "INSERT INTO `room_features`(`room_id`, `features_id`) VALUES (?,?)";
    if ($stmt = mysqli_prepare($con, $q3)) {
        foreach ($features as $f) {
            // Check if the feature exists in the features table
            $checkFeatureQuery = "SELECT id FROM features WHERE id = ?";
            $checkStmt = mysqli_prepare($con, $checkFeatureQuery);
            mysqli_stmt_bind_param($checkStmt, 'i', $f);
            mysqli_stmt_execute($checkStmt);
            mysqli_stmt_store_result($checkStmt);

            if (mysqli_stmt_num_rows($checkStmt) > 0) {
                // Feature exists, so insert it
                mysqli_stmt_bind_param($stmt, 'ii', $room_id, $f);
                mysqli_stmt_execute($stmt);
            } else {
                echo "Feature ID {$f} does not exist in the features table!";
                $flag = 0;
            }
        ...
        $flag = 0;
        die('Query cannot be prepared for features insert');
    }

    echo $flag ? 1 : 0; // Output based on success
}
...
if (isset($_POST['get_all_rooms'])) {
    ...
    while ($row = mysqli_fetch_assoc($res)) {
        ...
        <tr class='align-middle'>
            ...
            <td>
                <span class='badge rounded-pill bg-light text-dark t-font'>Adult: $row[adult]</span><br>
                <span class='badge rounded-pill bg-light text-dark t-font'>Children: $row[children]</span>
            </td>
            <td>$row[price]$</td>
            <td>$row[quantity]</td>
            <td>$status</td>
...
if (isset($_POST['get_room'])) {
    ...
    $data = json_encode(["roomdata" => $roomdata, "features" => $features, "facilities" => $facilities]);
    echo $data;
}
