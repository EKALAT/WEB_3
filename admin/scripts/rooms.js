...
function add_room() {
    ...
    Array.from(add_room_form.elements['features[]']).forEach(el => {
        if (el.checked) {
            features.push(el.value); // Now push the selected feature into the array
        }
    });
    ...
    Array.from(add_room_form.elements['facilities[]']).forEach(el => {
        if (el.checked) {
            facilities.push(el.value); // Now push the selected facility into the array
        }
    });

    // Append collected features and facilities to the FormData
...
function remove_room(img_id, room_id) {
